...
function renderBarChart(
  ...
  // Prepare data
  const data: { name: string; value: number; color: string; cid: string }[] = [];
...
export function renderRCVisChart(
  ...
  const svg = d3.select(sankeyContainer).append('svg').attr('width', width).attr('height', height);
...
.attr('stroke', (d: any) => colorMap[d.target.cid] || '#888')
  ...
  node.append('rect')
    ...
    .attr('fill', (d: any) => colorMap[d.cid] || '#666')
    .attr('stroke', '#222');
  ...
  node.append('title')
    .text((d: any) => `Round ${d.round + 1} • ${d.name}: ${d.value}`);
  ...
  if (userPath && userPath.length !== 0) {
    ...
    for (const s of userPath) {
      ...
      const link = (graph.links as any).find((l: any) => l.source.id === sourceId && l.target.id === targetId);
  ...
  // Pass ballots length for exhaustion analysis
  const ballots = { length: result.rounds[0] ? 
    result.rounds[0].continuingBallots + result.rounds[0].exhausted : 0 };
